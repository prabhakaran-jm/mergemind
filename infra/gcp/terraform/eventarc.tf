...
# Storage bucket object to track the function source code
resource "google_storage_bucket_object" "dbt_function_source" {
  name   = "dbt-trigger-function.zip"
  bucket = google_storage_bucket.dbt_function_source.name
  source = "${path.module}/cloud_function/dbt-trigger-function.zip"
}
